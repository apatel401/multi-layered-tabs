 ...
 */
function _copySass() {
  return gulp.src("src/css/*.scss").pipe(gulp.dest(".tmp/css"));
}
...
function _copyImages() {
  return gulp.src("src/img/**/*", {base: "src"}).pipe(gulp.dest(".tmp"));
}
...
const publishTest = gulp.series(
  _cleanTest,
  gulp.parallel(_copyImages, _copySass, _jsDev, _copyDevelopmentAssets),
  _devWebpack
);
publishTest.description =
  'Runs a "test build" to /test, for local development/qa';
...
const defaultBuild = gulp.series(
  gulp.parallel(_cleanDist, _bump),
  gulp.parallel(_copyImages, _compileLib, _copySass),
  _distWebpack
);
